...
class NewOrder extends React.Component {
  ...
  renderOrder = (key) => {
    ...
    return (
      <li className="row order-row" key={key}>
        ...
        <div className="col-2">
          <button className="btn btn-outline-danger delete-item-btn"
  ...
  render() {
    ...
    return (
      <div className="NewOrder">
        <h1>
          {Object.keys(orderEditing).length > 1 ? 'Edit Order' : 'New Order'}
        </h1>
          {Object.keys(orderEditing).length > 1 ? (<h3>Order ID: {orderEditing.id}</h3>) : ''}
